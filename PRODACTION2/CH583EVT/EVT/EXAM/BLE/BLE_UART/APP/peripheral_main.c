...
// Инициализация и настройка 32 МГц кварца
void Init_32MHz_Crystal(void) {
    // Установка ёмкости на 12 пФ
    HSECFG_Capacitance(HSECap_14p);
    // HSECFG_Capacitance(HSECap_18p);

    // Установка тока смещения на 100%
    HSECFG_Current(HSE_RCur_100);
     //HSECFG_Current(HSE_RCur_150);

    // Выбор источника тактового сигнала HSE (32 МГц)
   // SetSysClock(CLK_SOURCE_HSE_16MHz);

    // Дополнительная калибровка
    Calibration_LSI(Level_128); // Уровень 128 для наивысшей точности калибровки
}
...
int main(void)
{
     ...
     //SetSysClock(CLK_SOURCE_PLL_20MHz);// оптимально
...
//---------------------------------------------------------------------------------
 ...
 //----------------------------------ENABLE ANALOG PIN POWER--------------------------------------//
      ...
      GPIOB_ModeCfg( GPIO_Pin_21, GPIO_ModeOut_PP_20mA );//подсветка
      GPIOB_ModeCfg( GPIO_Pin_10, GPIO_ModeOut_PP_20mA );//реле
    ...
    //  GPIOA_ModeCfg(GPIO_Pin_4, GPIO_ModeOut_PP_20mA);// CS0 - DEBUG PIN
     ...
     // setup_temp = setup_temp_load;//загрузка по умолчанию температуры
